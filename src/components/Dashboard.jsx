...
const Dashboard = () => {
    ...
    // FUNGSI UTAMA: Ambil Data dari Database Ubuntu (VERSI ANTI-CRASH)
    ...
    const fetchAllData = async () => {
        ...
        try {
            ...
            // Langsung pasang ke State (Tanpa perlu ngitung manual lagi!)
            setUserData(user);
            setWallets(wallets || []);
            setStats({
                    ...
                    <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
                        <div>
                            ...
                            <h2 className="text-5xl font-black text-white italic tracking-tighter">
                                {showBalance ? `Rp ${(viewMode === 'total' ? stats.total_balance : (stats.income - stats.expense)).toLocaleString('id-ID')}` : 'Rp •••••••••'}
                            </h2>
                        </div>
                            ...
                            {viewMode === 'total' ? (
                                <>
                                    {wallets.map(w => (
                                        <div key={w.id} className="min-w-[160px] bg-white/5 border border-white/5 p-4 rounded-2xl">
                                            ...
                                            <p className="text-sm font-black text-slate-200">{showBalance ? `Rp ${parseFloat(w.balance).toLocaleString('id-ID')}` : '••••'}</p>
                                        </div>
                                    ))}
                                ...
                                <>
                                    <div className="min-w-[160px] bg-emerald-500/5 border border-emerald-500/10 p-4 rounded-2xl flex items-center gap-3">
                                        ...
                                        <div><p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Masuk</p><p className="text-sm font-black text-white italic">Rp {stats.income.toLocaleString('id-ID')}</p></div>
                                    </div>
                                    <div className="min-w-[160px] bg-rose-500/5 border border-rose-500/10 p-4 rounded-2xl flex items-center gap-3">
                                        ...
                                        <div><p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Keluar</p><p className="text-sm font-black text-white italic">Rp {stats.expense.toLocaleString('id-ID')}</p></div>
                                    </div>
                                </>
                            )}
                        </div>
                        ...
                        {transactions.slice(0, 10).map((trx) => (
                            <div key={trx.id} className="group flex flex-col md:flex-row md:items-center justify-between p-5 rounded-[2rem] hover:bg-white/[0.03] transition-all">
                                <div className="flex items-center gap-5">
                                    <div className={`p-4 rounded-2xl ${trx.type === 'income' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-rose-500/10 text-rose-400'}`}>
                                        {trx.type === 'income' ? <ArrowUpRight size={22} /> : <ArrowDownLeft size={22} />}
                                    </div>
                                    <div>
                                        {/* PENGAMANAN RENDER KATEGORI */}
                                        <h4 className="font-bold text-slate-200 group-hover:text-amber-500 transition-colors uppercase text-sm tracking-wide">
                                            {typeof trx.category === 'object' ? trx.category?.name : (trx.category || 'Tanpa Kategori')}
                                        </h4>
                                ...
                                <div className="mt-4 md:mt-0 md:text-right flex flex-row-reverse md:flex-col justify-between items-center md:items-end">
                                    <p className={`text-xl font-black italic ${trx.type === 'income' ? 'text-emerald-400' : 'text-rose-400'}`}> 
                                        {trx.type === 'income' ? '+' : '-'} Rp {parseFloat(trx.amount || 0).toLocaleString('id-ID')}
                                    </p>
